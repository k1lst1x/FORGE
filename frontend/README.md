...
frontend/
  ...
  console.html        the five-screen operator console
  app.html            the original React entry, kept
  public/
    landing.js        the hero and closing-CTA canvases
    console/          config.js, demo-data.js, chat-input.js, auth.js, app.js
  src/                React (App.tsx, main.tsx, styles.css)
  vite.config.ts      three-page build; the /api dev proxy is untouched
```
...
app.mount("/console", StaticFiles(directory="frontend/dist", html=True), name="console")
```
...
# http://localhost:5173/          -> landing (offline dataset)
# http://localhost:5173/console.html?api=http://localhost:8000
```
...
the `ChangeRequest` shape from `engine/models.py` as-is, accepts a bare array or
...
cd frontend
npm install
npm run dev      # landing at /, console at /console.html, React at /app.html
npm run build    # builds all three pages into dist/
```

Both of our pages are plain HTML that load their JS from `public/`, so Vite
copies them through untouched -- no compile step, no React involved. They also
still work opened straight from disk.

**One integration point left to settle.** The dev proxy rewrites `/api/x` to
`localhost:8000/x`, but forge-control serves `/api/x` unrewritten. Until those
agree, point the console at the API directly with
`?api=http://localhost:8000`, or set `apiBase` in `public/console/config.js`.
An unreachable API is not fatal: the console falls back to its bundled dataset
and says so with a DEMO DATA pill.
